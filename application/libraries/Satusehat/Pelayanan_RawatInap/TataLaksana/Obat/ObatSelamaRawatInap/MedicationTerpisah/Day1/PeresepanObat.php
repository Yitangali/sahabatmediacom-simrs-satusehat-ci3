...
class PeresepanObat {
    ...
    */
    ...
    public function createMedicationForRequest_Furosemide(array $payload): array {
        ...
        if (
            ...
            throw new \Exception('Gagal membuat Medication Furosemide atau response tidak valid');
        }

        // ekuivalen Postman:
        // pm.collectionVariables.set("Medication_ForRequest_FurosemideDay1", data.id)
        return [
            'id'           => $response['id'],
            'resourceType' => $response['resourceType'],
            'raw_response' => $response, // opsional (debug / logging)
        ];
    }


    public function createMedicationForRequest_InfusRingerLaktat(array $payload): array {
        $response = $this->client->request(
            'POST',
            'Medication',
            $payload
        ...
        if (
            ...
            throw new \Exception('Gagal membuat Medication Infus Ringer Laktat atau response tidak valid');
        }

        // ekuivalen Postman:
        // pm.collectionVariables.set("Medication_ForRequest_InfusDay1", data.id)
        return [
            'id'           => $response['id'],
            'resourceType' => $response['resourceType'],
